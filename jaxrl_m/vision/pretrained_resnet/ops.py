...
def freezed_batch_norm(x, epsilon=1e-05, momentum=0.99, params=None, dtype='float32'):
    if params is None:
        ...
        x = FreezedBatchNorm(epsilon=epsilon,
                      momentum=momentum,
                      bias_init=lambda *_ : jnp.array(params['bias']),
                      scale_init=lambda *_ : jnp.array(params['scale']),
                      mean_init=lambda *_ : jnp.array(params['mean']),
...
class FreezedBatchNorm(nn.Module):
    """BatchNorm Module.
    ...
    def __call__(self, x, use_running_average: Optional[bool] = None):
        ...
        if use_running_average:
            ...
            raise ValueError('use_running_average must be True for FreezedBatchNorm')
        ...
        if self.use_scale:
            scale = self.param('scale',
                               self.scale_init,
                               reduced_feature_shape).reshape(feature_shape)
            mul = mul * lax.stop_gradient(scale)
        y = y * mul
        if self.use_bias:
            bias = self.param('bias',
                              self.bias_init,
                              reduced_feature_shape).reshape(feature_shape)
            y = y + lax.stop_gradient(bias)
        return jnp.asarray(y, self.dtype)
