...
from jaxrl_m.networks.vae import VAEImageActionDecoder, VAEImageActionEncoder
from jaxrl_m.networks.mlp import MLP


class ImageActionVAEAgent(flax.struct.PyTreeNode):
    ...
    def update(self, batch: Batch, pmap_axis: str = None):
        def loss_fn(params, rng):
            ...
            recon_image = reconstructions[0]
            recon_action = reconstructions[1]

            recon_loss_image = ((recon_image - batch["image"]) ** 2).mean()
            recon_loss_action = ((recon_action - batch["actions"][:, :4].reshape(batch["actions"].shape[0], -1)) ** 2).mean()
            recon_loss = recon_loss_image + 0.01*recon_loss_action
            ...
            return (
                ...
                {
                    "vae_loss": vae_loss,
                    "image_recon_loss": recon_loss_image,
                    "action_recon_loss": recon_loss_action,
                    "kl_loss": kl_loss,
                    # "samples": samples,
                }
            )
    ...
    def get_debug_metrics(self, batch: Batch, seed: PRNGKey):
        ...
        recon_loss_image = ((recon_image - batch["image"]) ** 2).mean()
        recon_loss_action = ((recon_action - batch["actions"][:, :4].reshape(batch["actions"].shape[0], -1)) ** 2).mean()
        recon_loss = recon_loss_image + recon_loss_action
        # closed-form Gaussian KL from N(0, 1) prior
        kl_loss = (-0.5 * (1. + log_stds - means ** 2 - jnp.exp(log_stds)).sum(-1)).mean()
    ...
    def create(
        ...
        action_decoder = MLP(
            hidden_dims=[64, 64, 4 * 7],
            activations=nn.relu
        )

        networks = {
            "encoder": VAEImageActionEncoder(
                encoder,
                latent_encoder,
            ),
            "decoder": VAEImageActionDecoder(
                latent_decoder,
                decoder,
                action_decoder,
            )
        ...
        variables = model_def.init(init_rng, encoder=[observations["image"], observations["actions"]], decoder=[jnp.zeros((1, latent_kwargs["output_dim"]))])
